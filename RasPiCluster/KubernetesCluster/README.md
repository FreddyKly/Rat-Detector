...
At first install the [Raspberry Pi Imager](https://www.raspberrypi.com/software/) on your PC. Other applications do also work to flash the OS image on the micro SD memory cards, but this particular has two advantages. The imager has the Raspberry Pi operatingsystems already preselected. It also has an "advanced settings" menu, which can be used to give a hostname, activate SSH, give a username and a password, and set up the Wifi connection to your network. This means one onlye has to plug the micro SD card into the Raspberry after flashing it.
The operating system we are using is **Raspberry PI OS (32-BIT)**. The main reason for using a 32-Bit architecture is to reduce the overhead. Raspberry Pis have compared to the servers used in a datacenter low computational power.
...
The installation of the workernodes is almost identical to the installation fo the masternode. Install k3s with the following command and insert the masternode IP address and token in the marked fields:
...
To determine which of the two options works best, we split up the hardware. After a period of testing we got to the conclusion that MinIO created to much overhead and therefore the application kept crashing the cluster nodes. The NFS-Server worked without a problem. To enhance the available storage we made use of an external SSD hard drive. The goal of this enhancement was to reduce the number of read/writes on the micro SD card. We plugged the SSD into the masternode and bound the pod of the NFS-Server with a label to the node.

Now this method seems at first contradictive to our previous statement about the intended use of a Kubernetes cluster. But here we argue with the hardware limitations, a productive cluster could use a Network Attached Storage or one could configure predefined solutions for persistent storage. From our experience the Raspberries did not have the capacity to run MinIO and our applications. An NFS-Server is similar to a NAS and since we only have one masternode we are not creating more single-points of failure. Hence we decided us for the last option. 

It has to be stated that our decision between the NFS-Server and MinIO might have been biased. During the project (after our testing period) we swapped the micro SD cards of the MinIO nodes for newer ones. This lead to an notable performance increase. At this point in time we already build on our previous decision and a change was not possible anymore. Hence we sticked to the NFS-Server.
...
The password of the MariaDB is "password". It is saved in the "secret.yaml" and base64 encoded.
